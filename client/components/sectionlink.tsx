...
export function SectionLinkPopup({course, section, term, children}: {
	...
	const termId = useInfo().terms[term]!.id;
	...
	return <div className={`flex flex-col p-2 items-start ${isInTooltip ? "max-w-60" : ""}`} >
		...
		{section ? <>
			...
			<div className="flex flex-col gap-1 mt-3 items-start" >
				...
				<CatalogLinkButton href={`https://selfservice.mypurdue.purdue.edu/prod/bwckschd.p_disp_detail_sched?term_in=${termId}&crn_in=${section.crn}`} />
			</div>
		</> : children}
	</div>;
}
